...
int main() {
  ...
  if((unsigned int)targetHeight>=MAX_HEIGHT || (int)targetHeight<0 || (unsigned int)targetHeight>=height){
      cout << "Invalid target dimensions" << endl;
      return 0;
     
  }else if((unsigned int)targetWidth>=MAX_WIDTH || (int)targetWidth<0 || (unsigned int)targetWidth>=width){
    cout << "Invalid target dimensions" << endl;
    return 0;
